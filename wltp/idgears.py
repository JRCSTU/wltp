...
def _transient_filter_df(df, col, std_threshold=0.2, apply_dequantize=True):
    ...
    thres = (grad_mean - std_threshold * grad_std, grad_mean + std_threshold * grad_std)
...
def _filter_cycle_and_derive_ratios(df, filter_outliers=None, filter_transient=True):
    df["VoverN"] = (
        df.V / df.N
    )  # Work with VoverN because evenly spaced (not log), and 0 < VoverN < 1
...
def _approximate_ratios_by_binning(ratios, bins, ngears):
    ...
    peaks1 = peaks0[:ngears].sort(
        ["value"], ascending=True
    )  ## Keep #<gears> of most-populated bins.
    # display(peaks1)
    ...
    r_min -= gear_distance
    r_max += gear_distance
    peaks2, h_centers, h_points = find_histogram_peaks(
        ratios[(ratios >= r_min) & (ratios <= r_max)], bins
    )
    # display(peaks2)

    peaks3 = peaks2[:ngears].sort(
        ["value"], ascending=True
    )  ## Keep #<gears> of most-populated bins.
    # display(peaks3)
...
def _norm1_1d_vq(obs, guess, is_robust=False):
    ...
    if is_robust:  ## Does not make a difference!
        robust_prcntile = (
            4.685
        )  ##  Bisquare M-estimator with 95% efficiency under the Gaussian model.
        R_deleved = dist / (
            robust_prcntile * 1.4826 * np.median(dist)
        )  ## * sqrt(1 - hat_vector))
...
#
Detekt = namedtuple(
    "Detekt", ["distort", "guess", "final", "all_peaks_df", "hist_X", "hist_Y"]  ##
)
...
def _append_aproximate_Detekt(ngears, cases, ratios, all_peaks_df, hist_X, hist_Y):
    ...
    if ratios.size == ngears:
        ...
        cases.append(
            _new_Detekt_approximation(
                np.linspace(ratios.min(), ratios.max(), ngears),
                all_peaks_df,
                hist_X,
                hist_Y,
            )
...
def _gather_guessed_Detekts(ngears, cycle_df):
    ...
    for bins in bins_cases:
        all_peaks_df, peaks_df1, peaks_df2, hist_X, hist_Y = _approximate_ratios_by_binning(
            cycle_df.VoverN, bins=bins, ngears=ngears
        )

        _append_aproximate_Detekt(
            ngears,
            guessed_detekts,
            peaks_df1.value.values,
            all_peaks_df,
            hist_X,
            hist_Y,
        )
    ...
    guessed_detekts.append(
        guessed_detekts[0]._replace(
            guess=np.linspace(cycle_df.VoverN.min(), cycle_df.VoverN.max(), ngears)
        )
    )
...
def _gather_final_Detekts(ngears, cycle_df, guessed_gears):
    ...
    for gears in guessed_gears:
        ratios, distort = _estimate_ratios_by_kmeans(
            cycle_df.VoverN.values, gears.guess
        )
        if ratios.size == ngears:
...
def detect_gear_ratios_from_cycle_data(ngears, cycle_df):
    ...
    if not detekts[0].final is None:
        ...
        raise Exception(
            "Detection failed to estimate any gear-ratios!\n  All-Detekts(%s)" % detekts
        )
...
def reconstruct_engine_speed(cycle_df, gear_ratios):
    ...
    TODO: Class-method
    """
    cycle_df["GEARS_detekted"] = (
        identify_gears_from_ratios(cycle_df["VoverN"], gear_ratios) + 1
    )
...
def plot_idgears_results(cycle_df, detekt, fig=None, axes=None, original_points=None):
    ...
    if not fig:
        ...
        fig.suptitle(
            "Detekt: %s, Accuracy: %s (wltp-good: ~< 1e-3)"
            % (detekt.final, detekt.distort)
        )
    ...
    ax1.set_xlim(
        cycle_df.V.median() - 2 * cycle_df.V.mad(),
        cycle_df.V.median() + 2 * cycle_df.V.mad(),
    )
    ...
    ax2.plot(
        detekt.all_peaks_df.value,
        detekt.all_peaks_df.population,
        "ob",
        markersize=8,
        fillstyle="none",
    )
