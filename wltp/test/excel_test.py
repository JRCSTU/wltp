...
@skipIf(
    not ("darwin" in sys.platform or "win32" in sys.platform),
    "Cannot test xlwings in Linux",
)
class TestExcel(unittest.TestCase):
    ...
    def test_excel_refs(self):
        ...
        cases = [
            ...
            (
                "@{sheet}!E2:F2.table(strict=True, header=True)".format(
                    sheet=sheetname
                ),
                table,
            ),
        ...
        if errors:
            raise Exception(
                "There are %i out of %i errors!" % (len(errors), len(cases))
            )
