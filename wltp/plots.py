...
class MidPointNorm(Normalize):
    ...
    def __call__(self, value, clip=None):
        ...
        if not (vmin < midpoint < vmax):
            raise ValueError(
                "Midpoint(%s) must be between minvalue(%s) and maxvalue(%s)!"
                % (midpoint, vmin, vmax)
            )
        elif vmin == vmax:
            ...
            raise ValueError(
                "Maxvalue(%s) must be bigger than minvalue(%s)!" % (vmin, vmax)
            )
        else:
            vmin = float(vmin)
            vmax = float(vmax)
...
def plot_xy_diffs_scatter(
    ...
    if axes_tuple:
        ...
        twin_axis.set_ylabel(
            r"$\Delta %s$" % y_label.replace("$", ""), color=color_diff
        )
    ...
    ## Plot data
    #
    l_ref = axes.plot(X_REF, Y_REF, ref_fmt, label=ref_label, alpha=alpha, **ref_kws)
    ...
    l_diff = twin_axis.plot(
        line_points,
        polyval(regress_poly, line_points),
        "-",
        color=color_diff,
        label=diff_label,
    )
...
class DataCursor(object):
    ...
    def __init__(
        self, ax=None, text_template="x: {x:0.2f}\ny: {y:0.2f}\n{txt}", annotations=None
    ):
        ...
        if event.mouseevent.xdata is not None:
            try:
                ...
                self.annotation.set_text(
                    self.text_template.format(x=x, y=y, txt=annotation)
                )
