...
#  (might take 10'')
root_prov_info = vehdb.provenance_info(files=[heinzdb_fpath]) 
```
...
#
base = vehdb.provenance_info(files=[xlfname], base=root_prov_info)


def store_inputs_per_car(h5db, specs, c_vehnum=c_vehnum,  c_engno=c_engno,  c_SM=c_SM, c_ASM=c_ASM):
    for _, spec in specs.iterrows():
        vehnum = spec[c_vehnum]
        spec_node = vehdb.vehnode(vehnum, "iprop")
        pwot_node = vehdb.vehnode(vehnum, "pwot")
...
def store_results_per_car(
    ...
    for outfpath in results_dir.glob("gearshift_table_all.*.xlsx"):
        ...
        base = vehdb.provenance_info(base=root_prov_info, files=[outfpath])
        for vehnum, outdf in batchdf.groupby(c_vehnum):
            try:
                outdf = outdf.set_index('tim')
                outdf, props = vehdb.drop_scalar_columns(outdf, scalar_columns)
            except Exception:
                display(grid(outdf, fitcols=False))
                raise

            props_group = vehdb.vehnode(vehnum, props_group_suffix)
            ...
            cycle_group = vehdb.vehnode(vehnum, cycle_group_suffix)
...
#
vehnum = 14
inpsr, outdf, outsr = vehdb.load_vehicle(h5fname, vehnum, 'iprop', 'cycle', 'oprop')
display(inpsr, outsr, outdf.columns, vehdb.grid(outdf, fitcols=False))
```
