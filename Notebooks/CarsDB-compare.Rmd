...
def define_schenes():
    ...
    return [
        ## (label, caseno, zoom, pan, desc-UNUSED)
        ("dive(v001@@764): 1->2 gear too early", "v001, 45.0, 30.40, A"),
        ("dive(v001@903): 1st gear not reached", "v001, 45.0, 36.4, B"),
        ("lowPower(v001@1724)", "v001, 43.0, 69.6, B"),
        ("sameCar?(v023)", "v023, 1, 0, DC"),
        ("noPowerDives(v024@1540-80)", "v024, 26, 63.60, C"),
        ("noPowerDive(v024@1574)", "v024, 26, 66.4, C"),
        ("noPowerDive(v025@1574)", "v025, 35, 64, C"),
        ("lowPowerDive(v033)", "v033, 5, 72, "),
        ("lowPowerDive(v075)", "v075, 5, 72, Vehicle has too many insufficient powers"),
        ("g5Ok?(v075@1542)", "v075, 55, 62.4, Vehicle has too many insufficient powers"),
        ("lowPower(v101@)", "v075, 1, 0, D"),
        ("lowPower(v111@)", "v075, 1, 0, D"),
        ("lowPower(v124@)", "v075, 1, 0, D"),
        ("lowPower(v125@)", "v075, 1, 0, D"),
    ]

def grid(df):
    """Display a dynamic grid if given `df` too long. """
    display(vehdb.grid(df, fitcols=df.shape[1] < 12))
...
def display_diff_cycle(pyalgo, accdb):
    merged = merge_pyalgo_accdb(pyalgo, accdb)
    diffgears_idx = merged["pyalgo/g_max0"] != merged["accdb/g_max"]
...
Selections = widgets.HBox(
    [Case, Zoom, Pan, AxisScenes, IsPyAlgoGears, PyAlgoSignals, IsAccdbGears, AccDBSignals, Gear]
)
...
def refresh_tabs():
    ...
    AxisScenes.options = define_schenes()
...
def update_valid_signals(change):
    ...
    pyalgo, _, ok_gears, accdb, _ = load_load_interactive_case(case_name)
    ...
    pyalgo_columns = [
        ...
        if not c[0].startswith("ok_") and not c[0] == 't'
    ]
    PyAlgoSignals.options = pyalgo_columns

    accdb_columns = [
        c
        for c in accdb.select_dtypes(exclude=[np.object])
...
def recreate_fig():
    ...
    for i in range(3):
        ...
        fig = plt.figure(num="Compare AccDB <--> PyAlgo \"Inital Gear\"", figsize=(20, 12))
...
def plot_gear_flags(
    case, gear, zoom, pan, pyalgo_signals, accdb_signals, is_pyalgo_gears, is_accdb_gears
):
    fig.clear()
    ...
    diffgears_total = (cycle["g_max0"] != accdb_cycle["g_max"]).sum()

    pyalgo = cycle.loc[scale]
    accdb = accdb_cycle.loc[scale]
    diffgears_view = (pyalgo["g_max0"] != accdb["g_max"]).sum()
    merged = merge_pyalgo_accdb(pyalgo, accdb)
    ...
    if show_signals:
        ...
        if not merged.empty:
            merged.astype(np.float64).plot.line(ax=ax, linewidth=2, linestyle=":")
            ax.legend()
            ax2.legend()
