...
AccDB vehicles: 42, 46, 52, 53 & 90 have broken v_max, 48 have broken wot(ASM).
...
A new figure gets created everytime the next "widgets' cell runs.
...
def plot_gear_flags(case, gear, zoom, pan, accdb_signals, is_accdb_gears):
    ...
    if is_accdb_gears:
        accdb = accdb.loc[:, accdb_gears] * (np.arange(len(accdb_gears)) + 1)
        accdb[accdb == 0] = np.NAN
        accdb = accdb.dropna(how="all", axis=1).fillna(0)
        ax2 = accdb.plot.line(ax=ax, linewidth=2, linestyle="--", secondary_y=True)
...
#                      data_label="n in min-1", ref_label="AccDB",
# #             data_fmt="+k", data_kws={},
