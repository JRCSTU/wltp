...
def define_schenes():
    ...
    return [
        ...
        ("Dive@764: 1->2 gear too early", "v001, 45.0, 30.40, A"),
        ("Dive@903: 1st gear not reached", "v001, 45.0, 36.4, B"),
        ("bad_dsc@v007", "v007, 3, 72, C"),
        ("bad_dsc@v024", "v024, 3, 72, C"),
        ("bad_dsc@v025", "v025, 3, 72, C"),
        ("bad_dsc@v026", "v026, 3, 72, C"),
        ("bad_dsc@v033", "v033, 3, 72, C"),
        ("bad_dsc@v082: bad f_dsc", "v082, 3, 72, C"),
        ("bad_dsc@v112", "v112, 3, 72, C"),
        ("bad_dsc@v113", "v113, 3, 72, C"),
        ("bad_dsc@v114", "v114, 3, 72, C"),
    ]
...
#
out_specs = [
    (display_diff_gears, "DiffGear samples"),
...
out_specs2 = [(widgets.Output(), *rec) for rec in out_specs]

def update_tab_contents(change):
    if change.new is None:
        return
    out, func, _ = out_specs2[change.new]
...
Tab.children = [out for out, _, _ in out_specs2]
for i, (_, _, title) in enumerate(out_specs2):
...
def plot_gear_flags(
    ...
    ax2 = pd.concat((pyalgo["g_max0"], accdb["g_max"]), axis=1).plot.line(
    ...
    if show_signals:
        ...
        if not merged.empty:
            ax3 = merged.astype(np.float64).plot.line(ax=ax3, linewidth=2, linestyle=":")
            ax3.legend()
