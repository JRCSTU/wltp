...
def plot_gwots(ac_props, py_props, gwots, g, bottom_g, *, offset=2):
    ...
    ax = gwots.loc[v_range, (f"g{g}", "p_resist")].plot(
        linewidth=3
    )
...
ac_props, py_props, wot, gwots, n2vs = read_veh(48)
```
...
```{python}
ac_props, py_props, wot, gwots, n2vs = read_veh(25)
```
...
# gwots['v'] = gwots.index BRAK
```

```{python}
vmax.calc_v_max(gwots, ac_props.f0, ac_props.f1, ac_props.f2)
```
