...
def heinz_db_result_table_to_csv(
    heinz_results_glob,
    inp_vehs_df,
    cols_to_move,
    cols_to_rename,
    ...
    for heinz_fname in heinz_results_glob:
        ...
        try:
            ...
            assert (
                veh == heinz_out_df.ix[0, "vehicle_no"]
            ), "heinz-out[0, 0](%s) != fname-veh(%s)!" % (heinz_out_df.iloc[0, 0], veh)
            ...
            for col in steady_cols:
                ...
                assert (heinz_out_df[col] == cval).all(), (
                    "move-col(%s): %s != for all-heinz-out(%s)!"
                    % (col, cval, heinz_out_df[col])
                )
            ...
            for (col, ncol) in cols_to_rename:
                ...
                if ncol in inp_vehs_df.columns:
                    ...
                    if (
                        not pd.isnull(dval)
                        and heinz_out_df[col].dtype == inp_vehs_df[ncol].dtype
                        and cval != dval
                    ):
                        print(
                            "Warn: rename-col(%s->%s): %s != inp-vehs(%s)!"
                            % (col, ncol, cval, dval)
                        )
            ...
            for col in cols_to_move:
                ...
                if col in inp_vehs_df.columns:
                    ...
                    if (
                        not pd.isnull(dval)
                        and heinz_out_df[col].dtype == inp_vehs_df[col].dtype
                        and cval != dval
                    ):
                        print(
                            "WARN: move-col(%s): %s != inp-vehs(%s)!"
                            % (col, cval, dval)
                        )
            ...
            heinz_out_df = heinz_out_df.drop(
                steady_cols + cols_to_drop + [index_col], axis=1
            )
...
def post_proc_heinz_results():
    ...
    dfin = heinz_db_result_table_to_csv(
        heinz_results_glob, dfin, cols_to_move, cols_to_rename, cols_to_drop, index_col
    )
...
def reindex_db_vehicles():
    ...
    dfin1 = pd.read_excel(
        heinz_classes_excel, 0, index_col=None, skiprows=4, parse_cols="C:Z"
    )
    dfin2 = pd.read_excel(
        heinz_classes_excel, 1, index_col=None, skiprows=4, parse_cols="C:Z"
    )
    dfin3 = pd.read_excel(
        heinz_classes_excel, 2, index_col=None, skiprows=4, parse_cols="C:Z"
    )
    ...
    #
    dfout["description1"] = dfout["vehicle_no"].apply(lambda x: "%s: " % x) + dfout[
        "description1"
    ].apply(lambda x: str(x))
    dfout["vehicle_no"] = dfout.reset_index().index

    dfout = dfout.rename(columns={"category": "ID_cat", "description1": "Comment"})
    ...
    dfout = dfout.reindex(
        columns=[
            ...
            "ndv_7",
            "ID_cat",
            "user_def_driv_res_coeff",
            "user_def_power_curve",
            "f0",
            "f1",
            "f2",
            "Comment",
            ...
            "g_v_max",
            "n_norm_vmax",
            "n_vmax",  # Should have dropped them.
        ]
    )
