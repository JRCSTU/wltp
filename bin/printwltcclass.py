...
wltc_data_files = {
    "class1": ["class1-1-Low", "class1-2-Medium"],
    "class2": [
        "class2-1-Low",
        "class2-2-Medium",
        "class2-3-High",
...
def read_wltc_class(wltc_class, assert_files=False):
    ...
    if wltc_class not in wltc_data_files:
        raise ValueError(
            "Unknown WLTC-class(%s)!  One of (%s) expected."
            % (wltc_class, ", ".join(sorted(wltc_data_files.keys())))
        )

    mydir = os.path.dirname(__file__)
    class_part_names = [
        os.path.join(mydir, "data", "%s.csv" % pname)
    ...
    parts = []
    speed_profile = iterate_csv(
        class_part_names, extract_speed_column, extract_end_time
    )
