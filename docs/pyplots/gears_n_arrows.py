...
def data_meanN_gears(cls_num):
    ...
    vehdata = wltpdb._run_the_experiments(
...
def plot(cls_num):
    ...
    parts = zip(
        ...
        [
            dict(
                data_fmt="1",
                data_kws=dict(color="#0000ff", markersize=4.8),
                diff_kws=dict(linestyle=":", linewidth=4),
            ),
            dict(
                data_fmt="_",
                data_kws=dict(color="#0033cc", markersize=4.8, fillstyle="none"),
                diff_kws=dict(linestyle="-.", linewidth=4),
            ),
            dict(
                data_fmt="o",
                data_kws=dict(color="#006699", markersize=4.8, fillstyle="none"),
                diff_kws=dict(linestyle="-", linewidth=4),
            ),
            dict(
                data_fmt="+",
                data_kws=dict(color="#009966", markersize=4.8, fillstyle="none"),
                diff_kws=dict(linestyle="--", linewidth=4),
            ),
        ],
    )
    ...
    for (part, kws) in parts:
        ...
        axes_tuple, artist_tuple = plots.plot_xy_diffs_arrows(
            ...
            X_REF,
            Y_REF,
            data_label="Python, %s" % part,
            diff_label="Diffs %s" % part,
            diff_cmap=cm.cool,  # @UndefinedVariable
            title="%s: Python(arrow-heads) compared to AccessDb-2secAccel (arrow-tails)"
            % cls_name,
            x_label=r"Mean Gear",
...
if __name__ == "__main__":
    ...
    for ax in axes_tuple[1:]:
        ax.set_zorder(0.1)
        main_axes.set_zorder(1)
