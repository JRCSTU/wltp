...
def read_all_cycle_data(cycle_run_file, start_col_index=0):
    ...
    if len(set(cols) - set(df.columns)) != 0:
        if df.shape[1] != 2:
            raise ValueError(
                "Expected (at least) 2-column dataset with 'V' [km/h] and 'N' [rpm] columns, got: %s"
                % df.columns
            )
        else:
            m = df.median(axis=0)
            df.columns = (
                cols if m.iloc[0] > m.iloc[1] else reversed(cols)
            )  # Assume RPMs are bigger numbers.
    return df


file_results = [  # VoverN-DISTORT       DISTORT1           1/DISTORT
    (
        [0.00862961, 0.01414138, 0.02040305, 0.02705056, 0.03361355, 0.03913649],
        0.000776115495842,
        0.00102891505859,
        0.000623132178672,
    ),
    (
        [0.0077537, 0.01430799, 0.02039729, 0.02715475, 0.03377971, 0.03948467],
        0.000182951571423,
        0.000319414911246,
        0.993806101491,
    ),
    (
        [0.00773553, 0.01428506, 0.02039676, 0.02713253, 0.03377312, 0.03950773],
        0.00034170109883,
        0.000618105921847,
        0.370373235935,
    ),
    (
        [0.00701223, 0.01270195, 0.01852081, 0.0244521, 0.029762, 0.04085927],
        0.000412191410851,
        0.00137557327434,
        4.44380835405e-20,
    ),
...
class IdgearsTest(unittest.TestCase):
    def test_dequantize(self):
        df = pd.DataFrame([[1, 3], [1, 3], [2, 3], [2, 4], [2, 4]])
        ddf = idg.dequantize(df)
        exp = pd.DataFrame([[1, 3], [1.5, 3.333], [2, 3.666], [2, 4], [2, 4]])
    ...
    def test_identify_gears_ratios_and_iverse(self):
        for start_col_index, result in zip(range(0, 7, 2), file_results):
            ...
            gears, distorts = idg.identify_gears_from_ratios(
                cycle_df["V"] / cycle_df["N"], result[0]
            )
            self.assertEqual(gears.dtype, np.int64)
            distort = distorts.mean()
            self.assertTrue((0 <= gears).all())
            ...
            gears, distorts = idg.identify_gears_from_ratios(
                cycle_df["N"] / cycle_df["V"], 1 / np.array(result[0])
            )
            self.assertEqual(gears.dtype, np.int64)
            self.assertTrue((0 <= gears).all())
            self.assertTrue((gears < ngears).all())
